...
```shell
curl "https://subdomain.mybrightsites.com/api/v2.2.0/users/123/notes?token=GXzAxWkkyYLsESGQTU15" \
  -X POST \
...
```shell
curl "https://subdomain.mybrightsites.com/api/v2.2.0/users/579/notes/123?token=GXzAxWkkyYLsESGQTU15" \
  -X PUT \
  -H "Content-Type: application/json" \
  -d @- <<'EOF'
{
...
`PUT|PATCH /api/v2.2.0/users/:user_id/notes/:id`
