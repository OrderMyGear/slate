...
```shell
curl "https://subdomain.bsites-staging.com/api/v2.4.0/users/123/moas_email_approvers?token=GXzAxWkkyYLsESGQTU15" \
  -X POST \
...
```shell
curl "https://subdomain.bsites-staging.com/api/v2.4.0/users/579/moas_email_approvers/123?token=GXzAxWkkyYLsESGQTU15" \
  -X PUT \
  -H "Content-Type: application/json" \
  -d @- <<'EOF'
{
...
`PUT /api/v2.4.0/users/:user_id/moas_email_approvers/:id`
