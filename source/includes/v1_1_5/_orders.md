...
```shell
curl "https://subdomain.mybrightsites.com/api/v1.1.5/orders?token=GXzAxWkkyYLsESGQTU15"
```
...
```shell
curl "https://subdomain.mybrightsites.com/api/v1.1.5/orders/1?token=GXzAxWkkyYLsESGQTU15"
```
...
<div><strong>order[ship_date] </strong></div><div> optional </div> | <div>Ship Date</div><div>Valid format is <a href="https://en.wikipedia.org/wiki/ISO_8601" target="_blank">ISO 8601</a>, e.g. “YYYY-MM-DDThh:mm:ss”. You can provide a time-zone when using this parameter. If no time-zone is provided, then UTC will be used by default.</div>
<div><strong>order[in_hands_date] </strong></div><div> optional </div> | <div>In Hands Date</div><div>Valid format is <a href="https://en.wikipedia.org/wiki/ISO_8601" target="_blank">ISO 8601</a>, e.g. “YYYY-MM-DDThh:mm:ss”. You can provide a time-zone when using this parameter. If no time-zone is provided, then UTC will be used by default.</div>
...
```shell
curl "https://subdomain.mybrightsites.com/api/v1.1.5/orders/1/cancel?token=GXzAxWkkyYLsESGQTU15" \
  -X PUT
```
