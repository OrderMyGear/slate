...
```shell
curl "https://subdomain.mybrightsites.com/api/v2.6.0/products?token=GXzAxWkkyYLsESGQTU15"
```
...
```shell
curl "https://subdomain.mybrightsites.com/api/v2.6.0/products/579?token=GXzAxWkkyYLsESGQTU15"
```
...
<div><strong>product[note] </strong></div><div>optional , nil allowed</div> | <div>Product note</div><div>Must be a String</div>
...
`PUT|PATCH /api/v2.6.0/products/:id`
