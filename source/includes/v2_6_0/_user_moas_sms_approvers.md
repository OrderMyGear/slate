...
```shell
curl "https://subdomain.bsites-staging.com/api/v2.6.0/users/123/moas_sms_approvers?token=GXzAxWkkyYLsESGQTU15" \
  -X POST \
...
```shell
curl "https://subdomain.bsites-staging.com/api/v2.6.0/users/579/moas_sms_approvers/123?token=GXzAxWkkyYLsESGQTU15" \
  -X PUT \
  -H "Content-Type: application/json" \
  -d @- <<'EOF'
{
