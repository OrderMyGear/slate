...
```shell
curl "https://subdomain.bsites-staging.com/api/v2.3.0/users/123/notes?token=GXzAxWkkyYLsESGQTU15" \
  -X POST \
...
```shell
curl "https://subdomain.bsites-staging.com/api/v2.3.0/users/579/notes/123?token=GXzAxWkkyYLsESGQTU15" \
  -X PUT \
  -H "Content-Type: application/json" \
  -d @- <<'EOF'
{
...
`PUT /api/v2.3.0/users/:user_id/notes/:id`
